...
if [ "$DOCKERHUB_BUILD" == "true" ]; then
  PARALLEL_WORKERS="--parallel-workers 4"
fi

# ROS1 message setup
cd ~/.base-image/ros1_msgs_ws && source /opt/ros/noetic/setup.bash && colcon build $PARALLEL_WORKERS --packages-up-to autoware_msgs cav_msgs cav_srvs j2735_msgs can_msgs carma_debug_msgs autoware_lanelet2_msgs

# ROS2 message setup
cd ~/.base-image/ros2_msgs_ws && source /opt/ros/foxy/setup.bash && colcon build $PARALLEL_WORKERS --packages-up-to autoware_msgs autoware_lanelet2_msgs can_msgs carma_debug_ros2_msgs carma_driver_msgs carma_localization_msgs carma_msgs carma_perception_msgs carma_planning_msgs carma_v2x_msgs j2735_v2x_msgs
...
cd ../ 
sudo apt-get update 
colcon build $PARALLEL_WORKERS --packages-select ros1_bridge --cmake-args "--debug-output"
