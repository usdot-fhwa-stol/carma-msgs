...
colcon build --parallel-workers $(nproc) --cmake-args -DCMAKE_BUILD_TYPE=Release --packages-up-to \
    pcl_msgs \
...
time MAKEFLAGS="-j $MIN" colcon build --parallel-workers $(nproc) --event-handlers console_direct+ \
